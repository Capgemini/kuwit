...
function Wit(opts) {
  ...
  const continueRunActions = (sessionId, message, prevContext, res, i) => {
    return (json) => {
      if (i < 0) {
      ...
      if (json.type === 'msg') {
        ...
        return actions[action](request).then((ctx) => {
          ...
          return this.converse(sessionId, null, nextContext).then(
            continueRunActions(sessionId, message, nextContext, res, i - 1)
          );
        });
      } else {
  ...
  this.runActions = (sessionId, message, context, res, maxSteps) => {
    if (!actions) throwMustHaveActions();
    const steps = maxSteps ? maxSteps : DEFAULT_MAX_STEPS;
    return this.converse(sessionId, message, context, res).then(
      continueRunActions(sessionId, message, context, res, steps)
    );
  };
